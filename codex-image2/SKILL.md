...
`-ReasoningEffort` supports `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max` when the gateway/model accepts them. The default in this local configuration is `xhigh`.
